...
export function dibujarSubmundo(g, juego) {
  ...
  texto(g, mundo.nombre.toUpperCase(), 480, 82, 50);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    g.beginPath();
    g.arc(250, y, sel ? 44 : 36, 0, Math.PI * 2);
    g.fill();
    g.stroke();
    texto(g, abierto ? `${sub}` : "X", 250, y + 2, 30, "center", abierto ? "#1a100c" : "#2c221e");
    texto(g, nombres[i], 480, y - 12, sel ? 34 : 29, "center", abierto ? "#f4e3bd" : "#9b8a6c");
    const grado = juego.guardado.grados[claveSubnivel(juego.mundoSeleccionado, sub)];
    texto(g, grado ? `GRADO ${grado}` : abierto ? "SIN COMPLETAR" : "CANDADO", 480, y + 30, 21, "center", grado ? "#ffef9b" : "#d8a342");
  }
  texto(g, "ARRIBA/ABAJO: ELEGIR    ENTER: JUGAR    ESC: MAPA", 480, 640, 23);
}
...
export function dibujarTienda(g, juego) {
  ...
  ITEMS_TIENDA.slice(inicio, inicio + 4).forEach((it, idx) => {
    ...
    const tam = sel ? 31 : 25;
    texto(g, `${sel ? ">" : " "} ${it.nombre}  NIVEL ${nivel}/${it.max}  COSTO ${it.costo}`, 480, y, tam, "center", color);
  });
  ...
  g.beginPath();
  g.roundRect(160, 555, 640, 82, 8);
  g.fill();
  g.stroke();
  texto(g, item.desc, 480, 585, 22, "center", "#f4e3bd");
  texto(g, comprable ? "ENTER: COMPRAR" : nivel >= item.max ? "MEJORA AL MAXIMO" : "MONEDAS INSUFICIENTES", 480, 617, 20, "center", comprable ? "#b7e3a1" : "#8e8576");
  texto(g, "ARRIBA/ABAJO: ELEGIR    ESC: MAPA", 480, 675, 21);
}
